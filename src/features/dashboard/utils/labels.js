...
export const buildSummaryPayload = (profile, payments) => ({
  ...
  personal: {
    ...
    dob: profile?.dob,
    email: profile?.email,
    mobile: [profile?.mobileCountryCode, profile?.mobile].filter(Boolean).join(' '),
  ...
  professional: {
    ...
    speciality: specialityLabel(profile),
    otherAssociations: profile?.otherAssociations,
    refNo: profile?.refNo,
  },
  membership: {
    startDate: profile?.membershipStartDate,
    expiryDate: profile?.membershipExpiryDate,
    validityMonths: profile?.membershipValidityMonths,
    status: profile?.membershipStatus,
    isActive: profile?.membershipIsActive,
  },
  payment: {
    amountPaid: profile?.amountPaid,
    currency: profile?.currency,
    couponApplied: profile?.couponApplied,
